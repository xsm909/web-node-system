...
export const WorkflowList: React.FC<WorkflowListProps> = ({
    ...
    const [renameModalOpen, setRenameModalOpen] = React.useState<{ workflow: Workflow | null }>({ workflow: null });
    const [renameInputValue, setRenameInputValue] = React.useState('');
    const [deleteModalOpen, setDeleteModalOpen] = React.useState<{ workflow: Workflow | null }>({ workflow: null });
            ...
            cell: info => {
                ...
                return (
                    <div className="flex gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                            onClick={(e) => { e.stopPropagation(); setRenameModalOpen({ workflow: wf }); setRenameInputValue(wf.name); }}
                        ...
                        <button
                            onClick={(e) => { e.stopPropagation(); setDeleteModalOpen({ workflow: wf }); }}
    ...
    return (
        <div className="flex flex-col h-full bg-[var(--bg-app)] overflow-hidden">
            ...
            <ConfirmModal
                isOpen={!!renameModalOpen.workflow}
                title="Rename Workflow"
                description={`Enter a new name for "${renameModalOpen.workflow?.name}".`}
                confirmLabel="Rename"
                variant="success"
                onConfirm={() => {
                    if (renameModalOpen.workflow && renameInputValue.trim()) {
                        onRenameWorkflow({ ...renameModalOpen.workflow, name: renameInputValue });
                    }
                    setRenameModalOpen({ workflow: null });
                }}
                onCancel={() => setRenameModalOpen({ workflow: null })}
            >
                <input
                    autoFocus
                    className="w-full px-4 py-3 rounded-xl bg-[var(--bg-app)] border border-[var(--border-base)] text-sm text-[var(--text-main)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand transition-all font-medium"
                    ...
                    onKeyDown={(e) => {
                        if (e.key === 'Enter' && renameModalOpen.workflow && renameInputValue.trim()) {
                            onRenameWorkflow({ ...renameModalOpen.workflow, name: renameInputValue });
                            setRenameModalOpen({ workflow: null });
                        }
                    }}
            ...
            <ConfirmModal
                isOpen={!!deleteModalOpen.workflow}
                title="Delete Workflow"
                description={`Are you sure you want to delete "${deleteModalOpen.workflow?.name}"?`}
                confirmLabel="Delete"
                onConfirm={() => {
                    if (deleteModalOpen.workflow) {
                        onDeleteWorkflow(deleteModalOpen.workflow);
                    }
                    setDeleteModalOpen({ workflow: null });
                }}
                onCancel={() => setDeleteModalOpen({ workflow: null })}
            />
        </div>
    );
};
