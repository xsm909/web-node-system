 ...
 */
export const resolveValuePreview = (value: any): { display: string; isComplex: boolean } => {
    if (value === null || value === undefined) return { display: '', isComplex: false };

    // Handle Strings (SQL, MD, Python, etc.)
    if (typeof value === 'string') {
        const trimmed = value.trim();
        if (!trimmed) return { display: '', isComplex: false };
