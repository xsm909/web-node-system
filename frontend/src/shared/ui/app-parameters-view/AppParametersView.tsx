...
export const AppParametersView: React.FC<AppParametersViewProps> = ({
    ...
    return (
        <div className={`absolute top-0 right-0 w-full z-30 flex flex-col items-end pr-[20px] pointer-events-none ${className}`}>
            ...
            >
                {/* Main Glass Panel */}
                <div className="relative bg-surface-700/60 backdrop-blur-[5px] border-x border-b border-[var(--border-base)] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] rounded-bl-2xl rounded-br-none overflow-hidden flex flex-col">
                ...
                <div className="flex justify-end pr-0">
                    <button
                        onClick={onToggle}
                        className="bg-surface-700/04 backdrop-blur-[2px] border border-t-0 border-[var(--border-base)] shadow-[0_8px_16px_-6px_rgba(0,0,0,0.1)] rounded-b-xl px-6 py-1.5 flex items-center justify-center text-[var(--text-muted)] hover:text-brand transition-all hover:bg-brand/10 cursor-pointer pointer-events-auto group"
                    >
