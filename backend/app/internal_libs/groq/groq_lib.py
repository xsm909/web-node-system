...
def groq_create_new_conversation() -> str:
    conv_id = str(uuid.uuid4())
    _conversations[conv_id] = []
    _system_prompts[conv_id] = ""
    return conv_id

def groq_set_prompt(conversation_id: str, prompt: str) -> bool:
    if conversation_id not in _conversations:
        _conversations[conversation_id] = []
    _system_prompts[conversation_id] = prompt
    return True

def groq_ask_chat(conversation_id: str, text: str, model: str = "llama3-8b-8192") -> str:
...
def groq_ask_single(text: str, model: str = "llama3-8b-8192") -> str:
    api_key = _get_api_key(model)
    if not api_key:
        return "Error: GROQ_API_KEY not found in credentials."
...
class GroqAgentProvider(AgentProvider):
    def generate_response(
        ...
        # Groq often supports tool calling but we use standard completion for AgentStep loop
