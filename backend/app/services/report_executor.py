...
class ReportExecutor:
    ...
    def execute(self, parameters: dict, mode: str, execution_id: str = None):
        """
        ...
        try:
            ...
            if not params_proc_fn:
                # Optional, if missing just proceed with input parameters
                processed_params = parameters
                params_success = True
            else:
                processed_params, params_success = params_proc_fn(parameters, mode)

            if not params_success:
                return {
                    "success": False,
                    "error": "ParametersProcessing failed",
            ...
            result = gen_report_fn(processed_params)
            
            # Support both (data, success) and just data
